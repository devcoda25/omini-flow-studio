...
*/ class Language {
    ...
    */ isActiveAt(state, pos) {
        let side = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : -1;
...
*/ function ensureSyntaxTree(state, upto) {
    let timeout = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 50;
    var _a;
...
*/ function syntaxTreeAvailable(state) {
    let upto = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : state.doc.length;
    var _a;
...
*/ function forceParsing(view) {
    let upto = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : view.viewport.to, timeout = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 100;
...
*/ class LanguageDescription {
    ...
    */ static matchLanguageName(descs, name) {
        let fuzzy = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true;
...
*/ class IndentContext {
    ...
    */ countColumn(line) {
        let pos = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : line.length;
...
opening node, like this:
    ...
    foo(bar,
        baz)
*/ function delimitedIndent(param) {
    let { closing, align = true, units = 1 } = param;
...
*/ function continuedIndent() {
    let { except, units = 1 } = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {};
    return (context)=>{
...
function clearTouchedFolds(folded, from) {
    let to = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : from;
    let touched = false;
...
function announceFold(view, range) {
    let fold = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true;
    let lineFrom = view.state.doc.lineAt(range.from).number, lineTo = view.state.doc.lineAt(range.to).number;
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$codemirror$2f$view$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["EditorView"].announce.of("".concat(view.state.phrase(fold ? "Folded lines" : "Unfolded lines"), " ").concat(lineFrom, " ").concat(view.state.phrase("to"), " ").concat(lineTo, "."));
}
/**
...
function countCol(string, end, tabSize) {
    let startIndex = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : 0, startValue = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : 0;
    if (end == null) {
        end = string.search(/[^\s\u00a0]/);
    ...
    for (let name of tagStr.split(" ")){
        ...
        for (let part of name.split(".")){
            ...
            if (!value) {
                ...
                if (!found.length) warnForPart(part, "Modifier ".concat(part, " used at start of tag"));
                else found = found.map(value);
            } else {
                if (found.length) warnForPart(part, "Tag ".concat(part, " used as modifier"));
...
state.
*/ function findBlockComment(state, param, from, to) {
...
function changeBlockComment(option, state) {
    let ranges = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : state.selection.ranges;
...
on, if any.
*/ const cursorMatchingBracket = (param)=>{
    let { state, dispatch } = param;
...
*/ const selectMatchingBracket = (param)=>{
    let { state, dispatch } = param;
...
*/ const selectLine = (param)=>{
    let { state, dispatch } = param;
    let ranges = selectedLineBlocks(state).map((param)=>{
        let { from, to } = param;
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$codemirror$2f$state$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["EditorSelection"].range(from, Math.min(to + 1, state.doc.length));
    });
    dispatch(state.update({
...
syntax tree.
*/ const selectParentSyntax = (param)=>{
    let { state, dispatch } = param;
...
document.
*/ const deleteTrailingWhitespace = (param)=>{
    let { state, dispatch } = param;
    if (state.readOnly) return false;
    let changes = [];
...
*/ const moveLineUp = (param)=>{
    let { state, dispatch } = param;
    return moveLine(state, dispatch, false);
};
/**
Move the selected lines down one line.
*/ const moveLineDown = (param)=>{
    let { state, dispatch } = param;
...
*/ const deleteLine = (view)=>{
    ...
    let { state } = view, changes = state.changes(selectedLineBlocks(state).map((param)=>{
        let { from, to } = param;
        if (from > 0) from--;
...
*/ const insertNewlineKeepIndent = (param)=>{
    let { state, dispatch } = param;
...
*/ const selectNextOccurrence = (param)=>{
    let { state, dispatch } = param;
...
*/ class SearchQuery {
    ...
    */ getCursor(state) {
        let from = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0, to = arguments.length > 2 ? arguments[2] : void 0;
...
*/ const findNext = /*@__PURE__*/ searchCommand((view, param)=>{
    let { query } = param;
...
*/ const findPrevious = /*@__PURE__*/ searchCommand((view, param)=>{
    let { query } = param;
...
*/ const selectMatches = /*@__PURE__*/ searchCommand((view, param)=>{
    let { query } = param;
...
*/ const selectSelectionMatches = (param)=>{
    let { state, dispatch } = param;
...
*/ const replaceNext = /*@__PURE__*/ searchCommand((view, param)=>{
    let { query } = param;
...
replacement.
*/ const replaceAll = /*@__PURE__*/ searchCommand((view, param)=>{
    let { query } = param;
    if (view.state.readOnly) return false;
...
const Break = /[\s\.,:;?!]/;
function announceMatch(view, param) {
    let { from, to } = param;
    ...
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$codemirror$2f$view$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["EditorView"].announce.of("".concat(view.state.phrase("current match"), ". ").concat(text, " ").concat(view.state.phrase("on line"), " ").concat(line.number, "."));
}
const baseTheme = /*@__PURE__*/ __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$codemirror$2f$view$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["EditorView"].baseTheme({
    ".cm-panel.cm-search": {
        padding: "2px 6px 4px",
...
*/ function moveCompletionSelection(forward) {
    let by = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : "option";
    return (view)=>{
...
function findDiagnostic(diagnostics) {
    let diagnostic = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : null, after = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 0;
    let found = null;
...
function svg(content) {
    let attrs = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'viewBox="0 0 40 40"';
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$lezer$2f$json$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@lezer/json/dist/index.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$codemirror$2f$language$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@codemirror/language/dist/index.js [app-client] (ecmascript)");
;
;
/**
...
//# sourceMappingURL=node_modules_%40codemirror_dcecfe0f._.js.map